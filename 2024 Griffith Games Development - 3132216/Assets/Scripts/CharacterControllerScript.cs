...
public class CharacterControllerScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            anim.transform.localScale = new Vector3(1, 1, 1); // flip the animation to face right
        }
        if (Input.GetAxis("Horizontal") < -0.1f) // if the player is moving to the left
        {
            anim.transform.localScale = new Vector3(-1, 1, 1); // flip the animation to face left
        }
